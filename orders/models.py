...
class Order(models.Model):
    ...
    status              = models.CharField(max_length=120, default='created', choices=ORDER_STSTUS_CHOICES)
    ...
    def check_done(self):
        billing_profile = self.billing_address
        order_id = self.order_id
        shipping_address = self.shipping_address
        billing_address = self.billing_address
        total = self.total
        if billing_address and shipping_address and billing_address and order_id and total > 0:
            return True
        return False
